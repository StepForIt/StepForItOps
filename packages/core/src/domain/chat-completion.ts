/**
 * La fin de phrase proposée pendant la frappe, acceptée d'une touche.
 *
 * Déterministe et local : aucun appel IA. Une suggestion qui se paie une
 * seconde d'attente arrive après le mot suivant — trop tard pour servir —, et
 * elle coûterait un appel par frappe. Ce qu'on complète est donc ce qu'on
 * connaît déjà : les demandes courantes, celles que l'équipe a déjà écrites sur
 * ce workflow, et les noms de ses nœuds, qu'on ne retape jamais sans faute.
 *
 * La saisie tourne dans le navigateur, qui ne dépend pas de `@nwm/core` : la
 * règle y est recopiée (`apps/web/src/lib/chat-completion.ts`), et c'est ici
 * qu'elle est tenue et testée.
 */
